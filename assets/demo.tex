...
Both \texttt{lstlisting} and \texttt{minted} share a single unified float, \texttt{listing}, so numbering, styling, and \texttt{\textbackslash cref} behave identically no matter which highlighting engine you use for a given block. Wrap either environment in \texttt{\textbackslash begin\{listing\}[H] ... \textbackslash end\{listing\}}, with \texttt{\textbackslash caption\{\}} and \texttt{\textbackslash label\{\}} placed \emph{first}, before the code:
